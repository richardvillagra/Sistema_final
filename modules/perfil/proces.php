            ...
            if(isset($_POST['Guardar'])){
                if(isset($_POST['id_user'])){
                    ...
                    if(empty($_FILES['foto']['name'])){
                        ...
                        telefono = '$telefono' WHERE id_user = '$id_user'") or die ('error'.mysqli_error($mysqli));
                        if($query){
                        ...
                        if(in_array($extension, $allowed_extensions)){
                            if($tamaño_file <= 1000000){
                                if(move_uploaded_file($tmp_file, $path_file)){
                                        $query = mysqli_query($mysqli, "UPDATE usuarios SET username = '$username', name_user = '$name_user', email = '$email',
                                        telefono = '$telefono', foto = '$name_file' WHERE id_user = '$id_user'") or die ('error'.mysqli_error($mysqli));
                                    if($query){
                                        header("Location: ../../main.php?module=perfil&alert=1");
                                    }
                                }else{
